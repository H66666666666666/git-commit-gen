...
export function configCommand(): Command {
  ...
  cmd
    ...
    .description('Set AI provider (openai, claude, ollama)')
    .argument('<provider>', 'Provider name')
    .action((provider: string) => {
      if (!['openai', 'claude', 'ollama'].includes(provider)) {
        logger.error('Invalid provider. Use: openai, claude, or ollama');
        process.exit(1);
      }
      saveConfig({ provider: provider as 'openai' | 'claude' | 'ollama' });
